...
local getcustomassets = {
	['catnip/assets/wurst/triangle.png'] = 'rbxasset://wurst/triangle.png'
}
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/CatThatDrinksSprite/CatNipV4/'..readfile('catnip/profiles/commit.txt')..'/'..select(1, path:gsub('catnip/', '')), true) end)
...
function mainapi:CreateCategory(categorysettings)
	...
	function categoryapi:CreateModule(modulesettings)
		...
		triangle.Image = getcustomasset('catnip/assets/wurst/triangle.png')
